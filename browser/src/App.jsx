    ...
    <div class="min-h-screen bg-white text-black font-mono">
      <div class="max-w-7xl mx-auto px-4 py-8">
        <div class="text-center mb-12">
          <h1 class="text-4xl font-bold mb-4">Nostr Event Kinds Registry</h1>
        </div>
        ...
        <Show when={error()}>
          <div class="border-2 border-black p-6">Error: {error()}</div>
        </Show>
        ...
        <Show when={!loading() && !error()}>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div class="max-h-[70vh] overflow-y-auto">
                <table class="w-full">
                  <thead class="sticky top-0 bg-white border-b-2 border-black">
                    <tr>
                      <th class="px-4 py-2 text-left font-bold">Kind</th>
                      ...
                      {kind => (
                        <tr
                          class={`cursor-pointer hover:bg-gray-200 ${
                            selectedKind()?.number === kind.number ? "bg-gray-300" : ""
                          }`}
                          onClick={() => selectKind(kind)}
                        >
                          <td class="px-4 py-2">
                            <span class="font-bold">{kind.number}</span>
                          </td>
                          <td class="px-4 py-2">{kind.description}</td>
                        </tr>
                      )}
                    </For>
                  </tbody>
            ...
            <div class="bg-white border-2 border-black p-8 lg:sticky lg:top-8 self-start">
              ...
              >
                <h2 class="text-2xl font-bold mb-6">Kind {selectedKind().number}</h2>

                <div class="space-y-6">
                  <div>
                    <h3 class="text-sm font-bold mb-2">Description:</h3>
                    <p class="text-lg">{selectedKind().description}</p>
                  </div>

                  <Show when={selectedKind().tags && selectedKind().tags.length > 0}>
                    <div>
                      <h3 class="text-sm font-bold mb-3">Supported Tags:</h3>
                      <div class="space-y-2">
                        <For each={selectedKind().tags}>
                          {tag => <Tag tag={tag} kind={selectedKind()} />}
                        </For>
                      </div>
                    </div>
                  </Show>
                  ...
                  <div>
                    <h3 class="text-sm font-bold mb-2">Content Type:</h3>
